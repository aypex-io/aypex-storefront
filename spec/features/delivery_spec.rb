...
describe "Delivery", type: :feature, inaccessible: true, js: true do
  ...
  before do
    create(:product, stores: Aypex::Store.all) # product 2
  ...
  context "custom currency markers" do
    before do
      Aypex::Money.default_formatting_rules[:decimal_mark] = ","
      Aypex::Money.default_formatting_rules[:thousands_separator] = "."
    ...
    after do
      Aypex::Money.default_formatting_rules.delete(:decimal_mark)
      Aypex::Money.default_formatting_rules.delete(:thousands_separator)
    end
