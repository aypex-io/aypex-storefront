  ...
  describe 'change store' do
    ...
    context 'when show_store_selector preference is set to true' do
      ...
      context 'when there are more than one supported currencies' do
        ...
        let(:first_link_name) { "#{Aypex.t('i18n.this_file_language', locale: first_store.default_locale)} (#{first_store_currency_symbol})" }
        ...
        let(:second_link_name) { "#{Aypex.t('i18n.this_file_language', locale: second_store.default_locale)} (#{first_store_currency_symbol})" }
