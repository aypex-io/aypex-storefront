...
describe Aypex::OrdersController, type: :controller do
  ...
  context "Order model mock" do
    ...
    context "#empty" do
      ...
      it "redirects to aypex cart path" do
        expect(response).to redirect_to(aypex.cart_path)
      end
    end
    ...
    context "#update" do
      before do
        allow(user).to receive :last_incomplete_aypex_order
        allow(controller).to receive :set_current_order
      end

      it "cannot update a blank order" do
        put :update, params: {order: {email: "foo"}}
        expect(flash[:error]).to eq(Aypex.t(:order_not_found))
  ...
  context "line items quantity is 0" do
    let(:order) { create(:order, store: store) }
    let!(:line_item) { Aypex::Cart::AddItem.call(order: order, variant: variant).value }
