...
describe Aypex::CheckoutController, type: :controller do
  ...
  describe '#edit' do
    ...
    it 'redirects to the cart path unless checkout_allowed?' do
      ...
      expect(response).to redirect_to(aypex.cart_path)
    end
    ...
    context 'save successful' do
      ...
      before do
        ...
        allow(order).to receive(:available_shipping_methods).
          and_return [stub_model(Aypex::ShippingMethod)]
        allow(order).to receive(:available_payment_methods).
          and_return [stub_model(Aypex::PaymentMethod)]
      ...
      context 'with the order in the cart state' do
        ...
        it 'redirects the next state' do
          aypex_post_address
          expect(response).to redirect_to aypex.checkout_state_path('delivery')
        end
      end
      ...
      context 'with the order in the address state' do
        ...
        shared_examples 'new address created' do
          it 'creates new address' do
            expect { update }.to change { Aypex::Address.count }.by(1)
          end
        end
        ...
        shared_examples 'created address assigned to current user' do
          it 'assigns created address to current user' do
            ...
            expect(Aypex::Address.last.user_id).to eq user.id
          end
        end
        ...
        context 'with a billing and shipping address (with delivery step)' do
          ...
          context 'when addresses attributes are changed' do
            ...
            context 'when default address is not editable' do
              ...
              let(:bill_address_params) { build(:address, city: 'Chicago', state: state).attributes.merge(id: default_bill_address.id).except(:user_id, :created_at, :updated_at) }
              let(:ship_address_params) { build(:address, city: 'Washington', state: state).attributes.merge(id: default_ship_address.id).except(:user_id, :created_at, :updated_at) }
              let(:created_address_id) { Aypex::Address.find_by(city: 'Chicago').id }
          ...
          context 'when user is a guest' do
            ...
            context 'when submitted addresses does not exist' do
              ...
              it 'keeps created addresses unassigned' do
                ...
                expect(Aypex::Address.find_by(city: 'Chicago').user).to be_nil
                expect(Aypex::Address.find_by(city: 'Washington').user).to be_nil
              end

              it 'creates new addresses' do
                expect { update }.to change { Aypex::Address.count }.by(2)
              end
            end
      ...
      context 'when in the confirm state' do
        ...
        it 'redirects to the order view' do
          ...
          expect(response).to redirect_to aypex.order_path(order)
        end
  ...
  context 'When last inventory item has been purchased' do
    let(:product) { mock_model(Aypex::Product, name: 'Amazing Object') }
    let(:variant) { mock_model(Aypex::Variant) }
    let(:line_item) { mock_model Aypex::LineItem, insufficient_stock?: true, amount: 0 }
  ...
  context 'in the payment step' do
    ...
    let(:payment_method_id) { Aypex::PaymentMethod.first.id }
    ...
    context 'with store credits payment' do
      ...
      def expect_one_store_credit_payment(order, amount)
        ...
        expect(order.payments.first.source).to be_a Aypex::StoreCredit
      ...
      it 'can fully pay with store credits while removing an existing card' do
        credit_card = create(:credit_card, user: user, payment_method: Aypex::PaymentMethod.first)
    ...
    context 'remove store credits payment' do
      ...
      def expect_invalid_store_credit_payment(order)
        ...
        expect(order.payments.store_credits.with_state(:invalid).first.source).to be_a Aypex::StoreCredit
      end
  ...
  context 'Address Book' do
    ...
    before do
      Aypex::Cart::AddItem.call(order: order, variant: variant, quantity: 1)
      allow(controller).to receive(:aypex_current_user).and_return(user)
      allow(controller).to receive(:current_store).and_return(order.store)
      allow(order).to receive_messages user: user
      allow(order).to receive(:available_shipping_methods).and_return [stub_model(Aypex::ShippingMethod)]
      allow(order).to receive(:available_payment_methods).and_return [stub_model(Aypex::PaymentMethod)]
