  ...
  module FrontendHelper
    ...
    def define_image_method(style)
      self.class.send :define_method, "#{style}_image" do |product, *options|
        ...
        img = if image_path.present?
                ...
                inline_svg_tag 'aypex/frontend/aypex-logo.svg', class: 'noimage', size: '60px'
              end
    ...
    def logo(image_path = nil, options = {})
      ...
      image_path ||= if logo_attachment&.attached? && logo_attachment&.variable?
                       ...
                       asset_path('aypex/frontend/aypex-logo.svg')
                     end
      ...
      items = states.each_with_index.map do |state, i|
        ...
        if state_index < current_index
          ...
          link_content = if state == @order.state
                           ...
                           inline_svg_tag 'aypex/frontend/circle.svg', class: 'checkout-progress-steps-image'
                         end
          link_content << text
    ...
    def plp_and_carousel_image(product, image_class = '')
      ...
      image_url = if image.present?
                    ...
                    asset_path('aypex/frontend/aypex-logo.svg', width: '30px')
                  end
    ...
    def aypex_frontend_svg_tag(file_name, options = {})
      prefixed_file = "aypex/frontend/#{file_name}"
    ...
    def price_filter_values
      ActiveSupport::Deprecation.warn(<<-DEPRECATION, caller)
        `FrontendHelper#price_filter_values` is deprecated and will be removed in Aypex 5.0.
    ...
    def static_filters
      @static_filters ||= Aypex::Frontend::Config[:products_filters]
    end

    def additional_filters_partials
      @additional_filters_partials ||= Aypex::Frontend::Config[:additional_filters_partials]
    end
    ...
    def aypex_social_link(service)
      ...
      link_to "https://#{service}.com/#{current_store.send(service)}", target: :blank, rel: 'nofollow noopener', 'aria-label': service do
        content_tag :figure, id: service, class: 'px-2' do
          aypex_frontend_svg_tag("#{service}.svg", width: 22, height: 22)
        end
      end
    end
    ...
    def product_description(product)
      description = if Aypex::Frontend::Config[:show_raw_product_description] || product_wysiwyg_editor_enabled?
                      product.description
                    else
    ...
    def checkout_edit_link(step = 'address', order = @order)
      ...
      link_to aypex.checkout_state_path(step), class: classes, method: :get do
        inline_svg_tag 'aypex/frontend/edit.svg'
      end
    end
