module Aypex
  ...
  # is waranted.
  class CheckoutController < Aypex::StoreController
    ...
    def load_order_with_lock
      ...
      redirect_to(aypex.cart_path) && return unless @order
    end
    ...
    def ensure_checkout_allowed
      redirect_to aypex.cart_path unless @order.checkout_allowed?
    end

    def ensure_order_not_completed
      redirect_to aypex.cart_path if @order.completed?
    end
    ...
    def completion_route(custom_params = {})
      aypex.order_path(@order, custom_params.merge(locale: locale_param))
    end
    ...
    def before_address
      ...
      @order.ship_address ||= Address.new(country: current_store.default_country, user: try_aypex_current_user) if @order.checkout_steps.include?('delivery')
    end
