  ...
  module Checkout
    module AddressBook
      ...
      def normalize_addresses
        ...
        if params[:save_user_address].present? && try_aypex_current_user.present?
          [bill_address, ship_address].each { |address| address&.update_attribute(:user_id, try_aypex_current_user&.id) }
        end
